...
def classify(df: pd.DataFrame, score_col: str) -> pd.DataFrame:
    out = df.copy()
    # SLIGHT_SELL tightened from -0.020 to -0.026 (proportional to score_luck.py -0.050->-0.065)
    conds = [
        out[score_col] >= 0.040,
        out[score_col] >= 0.020,
        out[score_col] <= -0.040,
...
def report(merged_df: pd.DataFrame):
    ...
    WIDTH = 148
    print("\n" + "=" * WIDTH)
    print("WITHIN-SEASON BACKTEST v10 -- FULL PRODUCTION STACK")
    ...
    v10_ov = s5["OVERALL"][2]
    v9_ov  = 0.815   # known v9 result
    print(f"\n  Full v10 overall:  {v10_ov:.1%}")
    print(f"  vs v9 baseline:   {v10_ov - v9_ov:>+.1%}  (v9={v9_ov:.1%})")
    print(f"  vs v2 baseline:   {v10_ov - v2_ov:>+.1%}  (v2={v2_ov:.1%})")
    print(f"  Beats 81.5% v2/v9? {'YES' if v10_ov > 0.815 else 'NO'}")

    sl_v10 = s5["SLIGHT_SELL"][2]
    sl_v9_known = 0.750   # known v9 SLIGHT_SELL (2024 single year)
    sl_4yr_avg  = 0.620   # 4yr avg from multi-year backtest
    if not pd.isna(sl_v10):
        print(f"\n  SLIGHT_SELL accuracy (v10): {sl_v10:.1%}")
        print(f"  vs v9 (2024):               {sl_v10 - sl_v9_known:>+.1%}  (v9={sl_v9_known:.1%})")
        print(f"  vs 4yr avg:                 {sl_v10 - sl_4yr_avg:>+.1%}  (4yr={sl_4yr_avg:.1%})")
        print(f"  Threshold tightening helped? {'YES' if sl_v10 > sl_4yr_avg else 'NO'}")

    sl_buy_v10 = s5["SLIGHT_BUY"][2]
    sl_buy_v2  = 0.667
    if not pd.isna(sl_buy_v10):
        print(f"\n  SLIGHT_BUY accuracy: {sl_buy_v10:.1%}  vs v2: {sl_buy_v10 - sl_buy_v2:>+.1%}  "
              f"(target: >70%)")
...
def main():
    ...
    out_path = BASE_DIR / "backtest_results_within_season_v10.csv"
