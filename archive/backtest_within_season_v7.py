"""
Within-Season Backtest v7
=========================
v6 with EV trend threshold tightened from 2.0 mph to 1.5 mph below career avg.
...
Layer stack:
  ...
  Layer 3 -- EV trend:   current EV < career by >1.5 mph (was 2.0) & buy -> x0.85
...
Key questions vs v6:
  1. Does 1.5 mph threshold fire on more players and move any across bucket boundaries?
  2. Does overall accuracy exceed 81.5%?
"""
...
RTM_BASELINE   = 0.682
EV_THRESHOLD   = 1.5   # mph below career avg to trigger dampening (was 2.0 in v6)
...
def compute_layer1(april_df: pd.DataFrame, team_map_df: pd.DataFrame) -> pd.DataFrame:
    ...
    Computes sweet_spot_rate and avg_exit_velocity from the same window
    (temporally clean -- no lookahead into May-July).
    """
    df = april_df.copy()
    ...
    # BBE for sweet spot + exit velocity
...
def report(merged_df: pd.DataFrame):
    ...
    # ---- v6 EV counts for comparison (2.0 mph) ----
    ev_fired_v6 = (merged_df['ev_delta'].notna() & (merged_df['ev_delta'] < -2.0)).sum()
    ev_fired_v7 = (merged_df['ev_delta'].notna() & (merged_df['ev_delta'] < -EV_THRESHOLD)).sum()

    WIDTH = 128
    print("\n" + "=" * WIDTH)
    print("WITHIN-SEASON BACKTEST v7 -- EV THRESHOLD 1.5 mph (vs 2.0 mph in v6)")
    ...
    print(f"  EV trend fires at 1.5 mph (v7): {ev_fired_v7} players  "
          f"(+{ev_fired_v7 - ev_fired_v6} additional)")

    # Show who was newly caught by the tighter threshold
    newly_caught = merged_df[
        merged_df['ev_delta'].notna() &
        (merged_df['ev_delta'] < -EV_THRESHOLD) &
        (merged_df['ev_delta'] >= -2.0) &
        (merged_df['luck_score_L2'] > 0)
    ].copy()

    if len(newly_caught) > 0:
        print(f"\n  Players newly dampened by 1.5 mph threshold (not caught at 2.0 mph):")
        for _, row in newly_caught.iterrows():
            pid = int(row['batter'])
            l2  = row['luck_score_L2']
            l3  = row['luck_score_L3']
            d4_sig = d3.loc[d3['batter'] == pid, 'signal'].values
            sig = d4_sig[0] if len(d4_sig) else '?'
            print(f"    batter={pid}  ev_delta={row['ev_delta']:+.2f} mph  "
                  f"L2={l2:+.4f} -> L3={l3:+.4f}  signal={sig}")
    ...
    hdr = (
        ...
        f"{'--- L1+2+3: +EV 1.5mph ---':^26}  "
        f"{'--- L1+2+3+4: Full v7 ---':^26}  "
        f"{'--- Phase C Only ---':^22}"
    )
    sub = (
        f"{'Signal':<14}  "
    ...
    # ---- EV tier breakdown ----
    print(f"\n{'=' * WIDTH}")
    print(f"EV TREND DETAIL -- buy-signal players by delta vs career (1.5 mph threshold)")
    print(f"{'=' * WIDTH}")
    ...
    # ev_delta is already in d3 (classify copies all columns from merged_df)

    def ev_tier(delta):
        if pd.isna(delta): return 'No career baseline'
        if delta < -3.0:  return '< -3.0 mph (severe)'
        if delta < -1.5:  return '-3.0 to -1.5 mph (dampened at 1.5)'
        if delta < -1.0:  return '-1.5 to -1.0 mph (not caught)'
        if delta < 0:     return '-1.0 to 0 mph (slight dip)'
    ...
    tier_order = [
        '< -3.0 mph (severe)',
        '-3.0 to -1.5 mph (dampened at 1.5)',
        '-1.5 to -1.0 mph (not caught)',
        '-1.0 to 0 mph (slight dip)',
        '0 to +2.0 mph (stable/up)',
        '> +2.0 mph (trending up)',
        'No career baseline',
    ]
    print(f"  {'EV Tier':<36} {'N':>5} {'Correct':>8} {'Acc':>8} {'vs RTM':>8}")
    print(f"  {'-' * 68}")
    for tier in tier_order:
        ...
        acc = c / n
        print(f"  {tier:<36} {n:>5} {c:>8} {acc:>7.1%} {acc - RTM_BASELINE:>+8.1%}")

    # ---- Phase C breakdown ----
    print(f"\n{'=' * WIDTH}")
    print("PHASE C DETAIL -- accuracy by pattern type (full v7, non-flat outcomes)")
    print(f"{'=' * WIDTH}")
    ...
    # ---- Signal migration L1 -> v7 ----
    print(f"\n{'=' * WIDTH}")
    print("SIGNAL MIGRATION -- bucket changes from L1 baseline to Full v7")
    print(f"{'=' * WIDTH}")
    mig = (
    ...
    if len(changed) > 0:
        print(f"\n  {'L1 Signal':<16} -> {'v7 Signal':<16}  Count")
        print(f"  {'-' * 50}")
        for (l1s, l4s), grp in changed.groupby(['L1', 'L4']):
            print(f"  {l1s:<16} -> {l4s:<16}  {len(grp)}")

    # ---- Mean wOBA change ----
    print(f"\nMEAN wOBA CHANGE BY SIGNAL (full v7, no flat filter):")
    ...
    v7_ov = s4['OVERALL'][2]
    v6_ov = 0.808   # known v6 result for reference
    print(f"\n  Full v7 overall: {v7_ov:.1%}  vs v6: {v7_ov - v6_ov:>+.1%}")
    print(f"  Full v7 > 81.5%? {'YES' if v7_ov > 0.815 else 'NO'}")
...
def main():
    print("Within-Season Backtest v7 -- EV Threshold 1.5 mph")
    print("=" * 70)
    ...
    print("\nApplying Layer 3 (EV trend, threshold=1.5 mph)...")
    ...
    out_path = BASE_DIR / "backtest_results_within_season_v7.csv"
