"""
Multi-Year Within-Season Backtest v4
=====================================
Identical to v3 but with age-adjusted BABIP baselines.

  After mapping each batter's career BABIP, apply a decay multiplier based on
  age at the start of each backtest year (year - birth_year):
    age 32-34: x0.97   age 35-36: x0.94
    age 37-38: x0.91   age 39+:   x0.88
    Under 32: no adjustment

  Only fires when a career baseline exists (not the flat 0.300 fallback).
  Birth years come from career_stats (already loaded for EV adjustment in v3).

v3 benchmarks (for comparison):
  Overall 4yr avg: 84.1%  |  SLIGHT_SELL: 72.0%  |  BUY_LOW: 94.1%  |  vs RTM: +15.9pp
"""
...
def main():
    print("Multi-Year Within-Season Backtest v4  (2022-2025) -- Age-Adjusted BABIP Baselines")
    print("=" * 76)
    ...
    # ── Main table ────────────────────────────────────────────────────
    print(f"\n{'=' * 76}")
    print("ACCURACY TABLE -- April signals -> May-July outcomes  (v4: age-adjusted BABIP)")
    print(f"{'=' * 76}")
    print_table(results)
    ...
    # ── v3 vs v4 comparison ───────────────────────────────────────────
    print(f"\n{'=' * 76}")
    print("v3 vs v4 COMPARISON  (v3 = career BABIP baseline | v4 = age-adjusted BABIP baseline)")
    print(f"{'=' * 76}")
    ...
    print(f"\n  {'Metric':<22} {'v3':>8} {'v4':>8} {'Delta':>8}")
    print(f"  {'-' * 50}")
    for label, v3, v4 in [
        ("Overall 4yr avg",    V3_OVERALL, ov_avg),
        ("BUY_LOW 4yr avg",    V3_BL,      bl_avg),
        ("SLIGHT_SELL 4yr avg",V3_SS,      ss_avg),
        ("vs RTM 4yr avg",     V3_RTM,     rtm_avg),
    ]:
        delta = v4 - v3
        arrow = " <-- improved" if delta > 0.002 else (" <-- worse" if delta < -0.002 else "")
        print(f"  {label:<22} {v3:>7.1%} {v4:>7.1%} {delta:>+7.1%}{arrow}")
    ...
    # Q1: Does age adjustment help overall?
    beats_v3 = ov_avg > V3_OVERALL + 0.002
    hurts_v3 = ov_avg < V3_OVERALL - 0.002
    print(f"\n  Q1: Does age-adjusted BABIP improve overall accuracy vs v3 ({V3_OVERALL:.1%})?")
    print(f"      v3={V3_OVERALL:.1%}  v4={ov_avg:.1%}  delta={ov_avg-V3_OVERALL:>+.1%}")
    print(f"      Per-year: " + " | ".join(f"{r['year']}={r['stats']['OVERALL'][2]:.1%}" for r in results))
    if beats_v3:
        ans = "YES -- age adjustment improves prediction accuracy"
    elif hurts_v3:
        ans = "NO -- age adjustment hurts accuracy"
    else:
        ans = "NEUTRAL -- within noise margin (+/- 0.2%)"
    print(f"      Answer: {ans}")

    # Q2: Did BUY_LOW recover from v3's -0.9pp?
    bl_beats = bl_avg > V3_BL + 0.002
    bl_hurts = bl_avg < V3_BL - 0.002
    print(f"\n  Q2: Did BUY_LOW recover from v3's -0.9pp vs v2 ({V3_BL:.1%})?")
    print(f"      v3={V3_BL:.1%}  v4={bl_avg:.1%}  delta={bl_avg-V3_BL:>+.1%}")
    ...
    if bl_beats:
        ans = "YES -- BUY_LOW improved over v3"
    elif bl_hurts:
        ans = "NO -- BUY_LOW declined further vs v3"
    else:
        ans = "NEUTRAL -- no meaningful BUY_LOW change vs v3"
    print(f"      Answer: {ans}")

    # Q3: SLIGHT_SELL accuracy
    ss_beats = ss_avg > V3_SS + 0.002
    print(f"\n  Q3: Does SELL_HIGH hold above 90% in v4 (v3={V3_SS:.1%})?")
    print(f"      v3={V3_SS:.1%}  v4={ss_avg:.1%}  delta={ss_avg-V3_SS:>+.1%}")
    print(f"      Per-year: " + " | ".join(f"{r['year']}={r['stats']['SLIGHT_SELL'][2]:.1%}" for r in results))
    print(f"      Answer: {'IMPROVED' if ss_beats else ('WORSE' if ss_avg < V3_SS - 0.002 else 'NEUTRAL')}")

    # Q4: Best year
    year_ov = [(r["year"], r["stats"]["OVERALL"][2]) for r in results]
    best_year, best_acc = max(year_ov, key=lambda x: x[1])
    print(f"\n  Q4: Which year has best accuracy in v4?")
    for yr, acc in year_ov:
        v3_acc = {2022: 0.758, 2023: 0.812, 2024: 0.795, 2025: 0.825}.get(yr, float("nan"))
        delta = acc - v3_acc if not pd.isna(v3_acc) else float("nan")
        d_str = f"  (v3={v3_acc:.1%}  delta={delta:>+.1%})" if not pd.isna(v3_acc) else ""
    ...
    print("SUMMARY")
    print(f"{'=' * 76}")
    print(f"  Overall 4yr avg:    {ov_avg:.1%}  (v3={V3_OVERALL:.1%}  delta={ov_avg-V3_OVERALL:>+.1%})")
    print(f"  BUY_LOW 4yr avg:    {bl_avg:.1%}  (v3={V3_BL:.1%}  delta={bl_avg-V3_BL:>+.1%})")
    print(f"  SLIGHT_BUY 4yr avg: {sb_avg:.1%}")
    print(f"  SLIGHT_SELL 4yr avg:{ss_avg:.1%}  (v3={V3_SS:.1%}  delta={ss_avg-V3_SS:>+.1%})")
    print(f"  vs RTM 4yr avg:     {rtm_avg:.1%}pp  (v3={V3_RTM:.1%}pp  delta={rtm_avg-V3_RTM:>+.1%}pp)")
    print(f"  Total player-seasons evaluated: {sum(r['n_eval'] for r in results)}")

    verdict = "SHIP IT" if beats_v3 else ("HOLD -- no improvement" if hurts_v3 else "MARGINAL -- within noise")
    print(f"\n  Age-adjusted BABIP verdict: {verdict}")
